...
def path_builder(path_list):
    path = ''
    if len(path_list) == 1: return '/' # just a path for a root directory
    for folder in path_list:
        if len(path) == 0: path = folder
        elif path[-1] == '/': path += folder
        else: path += '/' + folder
    return path
    ...
    for line in f.readlines():
        if '$ cd ' in line:
            if ' ..' not in line:
                ...
                folder_path.append(current_dir)
                whole_path = path_builder(folder_path)
                if whole_path not in dir.keys():
                    dir[whole_path] = 0
            else:
                current_dir = folder_path.pop()
                whole_path = path_builder(folder_path)
        elif line[0].isdigit():
            size = int(line.split()[0])
            for p in range(len(folder_path)-1,0,-1):
                previous_path = path_builder(folder_path[:p])
                dir[previous_path] += size
            dir[whole_path] += size
    sizes = [v for k,v in dir.items()]
    part1 = sum(s for s in sizes if s <= 100000)
    part2 = min(s for s in sizes if s >= max(sizes) - 40000000)
    print(part1,part2)
