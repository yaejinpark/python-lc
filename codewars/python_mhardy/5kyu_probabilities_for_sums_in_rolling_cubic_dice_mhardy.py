...
# We want to know the probability of having the sum of the results equals to 11. For that result we have 
# only the combination of 6 and 5. So we will have two events: {5, 6} and {6, 5}
...
def rolldice_sum_prob(target, dice_amount):
    if target < dice_amount or target > 6*dice_amount:
        return 0
    good_sums = []
    sum_list = list(itertools.product(range(1,7),repeat=dice_amount))
    for roll in sum_list:
        if sum(roll) == target:
            good_sums.append(roll)
    return float(len(good_sums))/len(sum_list)
