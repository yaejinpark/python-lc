-- Task
-- Given the information about sales in a store, calculate the total 
-- revenue for each day, month, year, and product.
...
SELECT p.name AS product_name, EXTRACT(YEAR FROM s.date) AS year, EXTRACT(MONTH FROM s.date) AS month,
  EXTRACT(DAY FROM s.date) AS day, SUM(p.price * sd.count) AS total
FROM sales_details AS sd
LEFT JOIN sales AS s ON sd.sale_id = s.id
LEFT JOIN products AS p ON sd.product_id = p.id
