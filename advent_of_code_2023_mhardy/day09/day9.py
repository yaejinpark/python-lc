...
# 3 each step, the first sequence of differences that you generate will be 3 3 3 3 3. 
# Note that this sequence has one fewer value than the input sequence because at each step 
# it considers two numbers from the input. Since these values aren't all zero, repeat the 
# process: the values differ by 0 at each step, so the next sequence is 0 0 0 0. 
# This means you have enough information to extrapolate the history! Visually, 
...
#         hist_list.append(lst[i+1]-lst[i])
#     master_lst.append(hist_list)
#     for y in master_lst[-1]:
#         if y != 0:
#             return diff_reduce(master_lst[-1])
#         else:
#             pass
...
#     row = row.rstrip("\n")
#     master_lst = []
#     seq = [int(i) for i in row.split(" ")]
#     master_lst.append(seq)
#     diff_reduce(seq)
#     print(master_lst)
#     temp = master_lst[::-1]
#     for i, lst in enumerate(temp):
#         if i < len(temp)-1:
#             temp[i+1].append(temp[i+1][-1] + lst[-1])
#     new_lst = temp[::-1]
#     # print(new_lst)
#     total += new_lst[0][-1]
...
# beginning of your sequence of zeroes, then fill in new first values for 
...
for row in file.readlines():
    row = row.rstrip("\n")
    master_lst = []
    seq = [int(i) for i in row.split(" ")][::-1]
    master_lst.append(seq)
    diff_reduce(seq)
    temp = master_lst[::-1]
    for i, lst in enumerate(temp):
