...
#     game = row.rstrip().split(': ')
#     game_id = int(game[0].strip('Game '))
#     roll_list = game[1].split('; ')
#     flag = True
...
#             if value.isnumeric():
#                 # print(f'{value} {cubes[i+1].rstrip(",")} cubes')
#                 if 'red' in cubes[i+1] and int(value) > 12:
#                     flag = False
#                 if 'green' in cubes[i+1] and int(value) > 13:
#                     print(f'{value} {cubes[i+1].strip(",")} cubes: INVALID')
#                     flag = False
#                 if 'blue' in cubes[i+1] and int(value) > 14:
#                     print(f'{value} {cubes[i+1].strip(",")} cubes: INVALID')
#                     flag = False
#     if flag:
#         total += game_id
    ...
    for roll in roll_list:
        ...
        for i, value in enumerate(cubes):
            if value.isnumeric():
                ...
                if 'red' in cubes[i+1] and int(value) > cube_dict['red']:
                    cube_dict['red'] = int(value)
                if 'green' in cubes[i+1] and int(value) > cube_dict['green']:
                    cube_dict['green'] = int(value)
                if 'blue' in cubes[i+1] and int(value) > cube_dict['blue']:
                    cube_dict['blue'] = int(value)
    total += (cube_dict['red']*cube_dict['green']*cube_dict['blue'])

print(total)

# BOOM, FIRST TIME GO, NO DEBUGGING!!!!
# CORRECT TOTAL 65371