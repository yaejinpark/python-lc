...
# wait for the ferry, maybe you can help us with our food production problem. 
...
# Every type of seed, soil, fertilizer and so on is identified with a number, but numbers are 
...
# through other categories until you can find its corresponding location number. In this example, the corresponding types are:
...
for row in file.readlines():
    first_in_row = row.split(" ")[0]
    row = row.rstrip("\n")

    if first_in_row == 'seeds:':
        seeds = [int(i) for i in row.split(" ") if i.isnumeric()]
    elif first_in_row == 'seed-to-soil':
        current_list = seed_to_soil
    elif first_in_row == 'soil-to-fertilizer':
        current_list = soil_to_fertilizer
    elif first_in_row == 'fertilizer-to-water':
        current_list = fertilizer_to_water
    elif first_in_row == 'water-to-light':
        current_list = water_to_light
    elif first_in_row == 'light-to-temperature':
        current_list = light_to_temperature
    elif first_in_row == 'temperature-to-humidity':
        current_list = temperature_to_humidity
    elif first_in_row == 'humidity-to-location':
        current_list = humidity_to_location
    elif first_in_row.isnumeric():
        row_list = [int(i) for i in row.split(" ")]
        current_list.append((row_list[0], row_list[1], row_list[2]))

res_list = []
for seed in seeds:

    res = seed
    for cell in seed_to_soil:
        if res >= cell[1] and res <= (cell[1] + cell[2]):
            res = (res - cell[1]) + cell[0]
            break
    for cell in soil_to_fertilizer:
        if res >= cell[1] and res <= (cell[1] + cell[2]):
            res = (res - cell[1]) + cell[0]
            break
    for cell in fertilizer_to_water:
        if res >= cell[1] and res <= (cell[1] + cell[2]):
            res = (res - cell[1]) + cell[0]
            break
    for cell in water_to_light:
        if res >= cell[1] and res <= (cell[1] + cell[2]):
            res = (res - cell[1]) + cell[0]
            break
    for cell in light_to_temperature:
        if res >= cell[1] and res <= (cell[1] + cell[2]):
            res = (res - cell[1]) + cell[0]
            break
    for cell in temperature_to_humidity:
        if res >= cell[1] and res <= (cell[1] + cell[2]):
            res = (res - cell[1]) + cell[0]
            break
    for cell in humidity_to_location:
        if res >= cell[1] and res <= (cell[1] + cell[2]):
            res = (res - cell[1]) + cell[0]
            break
    res_list.append(res)
...
for i in range(0, len(seeds), 2):
    seeds_long.append((seeds[i], seeds[i]+seeds[i+1]))
print(seeds_long)
for seed in seeds_long:
    seed_range = [i for i in range(seed[0],seed[1]+1)]
    res = seed[0]
    for cell in seed_to_soil:
        if res >= cell[1] and res <= (cell[1] + cell[2]):
            res = (res - cell[1]) + cell[0]
    ...
    for cell in humidity_to_location:
        if res >= cell[1] and res <= (cell[1] + cell[2]):
            res = (res - cell[1]) + cell[0]
            break
    print(f'Appending location for {seed}: {res}')
    res_list.append(res)
