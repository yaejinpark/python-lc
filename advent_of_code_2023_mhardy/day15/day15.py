...
# At one point, you pass a button with a faint snout mark and the label "PUSH FOR HELP". No wonder you were loaded into that trebuchet so quickly!
...
# pc=6 becomes 214.
# ot=7 becomes 231.
# In this example, the sum of these results is 1320. Unfortunately, the reindeer has stolen the page containing the expected verification 
...
# The Test list, below, should return a HASH total of 1320
...
# be sure to use the label maker to mark the lens with the label given in the beginning 
...
# when you do this. If there aren't any lenses in the box, the new lens goes all the 
...
# Within each box, lenses are listed from front to back; each lens is shown as its label 
# and focal length in square brackets.
...
for key, val in box_dict.items():
    ...
    if val:
        for idx, v in enumerate(val):
            temp_total += ((key+1) * (idx+1) * int(v[-1]))
    total += temp_total
