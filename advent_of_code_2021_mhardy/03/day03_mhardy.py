...
for i in range(len(row_lists[0])):
    gamma_rate+=str(mode([int(j[i]) for j in row_lists]))
print(gamma_rate)
epsilon_rate = ''.join(['1' if i=='0' else '0' for i in list(gamma_rate)])
print(epsilon_rate)
gamma_rate_dec, epsilon_rate_dec = int(gamma_rate,2), int(epsilon_rate,2)
print(f'Part 1 Solution: {gamma_rate_dec*epsilon_rate_dec}')

# PART 2
temp_lists=row_lists.copy()

from collections import Counter


for i in range(len(temp_lists[0])):
    if len(temp_lists)==1:
        break
    count = Counter(j[i] for j in temp_lists)
    if count['1']>=count['0']:
        common='1'
    else:
        common='0'
    temp_lists=[row for row in temp_lists if list(row)[i]==common]
print(temp_lists)

temp_lists2=row_lists.copy()

for i in range(len(temp_lists2[0])):
    if len(temp_lists2)==1:
        break
    count = Counter(j[i] for j in temp_lists2)
    if count['1']>=count['0']:
        common='0'
    else:
        common='1'
    temp_lists2=[row for row in temp_lists2 if list(row)[i]==common]
print(temp_lists2)
oxy_rate, co2_rate = int(temp_lists[0], 2), int(temp_lists2[0], 2)
print(f'Part 2 Solution: {oxy_rate*co2_rate}')