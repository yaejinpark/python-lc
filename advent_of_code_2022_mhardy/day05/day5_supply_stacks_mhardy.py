...
stack = [['R','G','J','B','T','V','Z'],['J','R','V','L'],['S','Q','F'],['Z','H','N','L','F','V','Q','G'],['R','Q','T','J','C','S','M','W'],['S','W','T','C','H','F'],['D','Z','C','V','F','N','J'],['L','G','Z','D','W','R','F','Q'],['J','B','W','V','P']]

# for i in file.readlines():

#     move_list = i.rstrip().split(' ')

#     num_bricks = int(move_list[1])
#     first_col = int(move_list[3])-1
#     second_col = int(move_list[5])-1
 
#     moving_nums = []
#     for idx, letter in enumerate(stack[first_col]):
#         if idx >= len(stack[first_col])-num_bricks:
#             moving_nums.append(letter)
    
#     moving_nums = moving_nums[::-1]
 
#     for i in moving_nums:
#         stack[second_col].append(i)
    
#     del stack[first_col][-num_bricks:]

# print(f"{stack[0][-1]}{stack[1][-1]}{stack[2][-1]}{stack[3][-1]}{stack[4][-1]}{stack[5][-1]}{stack[6][-1]}{stack[7][-1]}{stack[8][-1]}")
...
# quickly wipe it away. The crane isn't a CrateMover 9000 - it's a CrateMover 9001.
...
for i in file.readlines():
    ...
    # moving_nums = moving_nums[::-1] ------------- (THE ONLY LINE OF CODE THAT NEEDED TO BE COMMENTED OUT)
