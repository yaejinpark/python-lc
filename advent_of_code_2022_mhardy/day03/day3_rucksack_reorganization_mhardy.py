...
# contains the items hcsFMMfFFhFp. The only item type that appears in both compartments is lowercase p.
...
for i in file.readlines():
    ...
    char_list = list(i.rstrip())
    current_group.append(char_list)
    row_counter += 1

    if row_counter == 3:
        for i in current_group[0]:
            for j in current_group[1]:
                for h in current_group[2]:
                    if i == j and i == h:
                        matches.append(i)
